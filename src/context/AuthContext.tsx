  ...
  useEffect(() => {
    ...
    if (isPublicRoute) {
      ...
      if (isConnected && (pathname === "/login" || pathname === "/signup")) {
        ...
        console.log(
          "AuthContext - Redirecting authenticated user to:",
          targetPath
        );
    ...
    if (isOnboardingRoute && isConnected) {
      console.log(
        "AuthContext - On onboarding route and authenticated, allowing access"
      );
      return;
    }

    console.log(
      "AuthContext - Not on public/onboarding route, checking auth..."
    );
